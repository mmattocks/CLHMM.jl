...
observations=zeros(Int64,100,1001)
...
function test_bw(hmm::HMM{Univariate,Float64}, observations::Matrix{Int64}, obs_lengths::Vector{Int64})
    ...
    for t in maximum(obs_lengths)-1:-1:1
        omask = mask[:,t+1]
        lls = ll_func(hmm,observations[:,t+1])
        ...
        for m in 1:N
            ...
            for i in 1:N
                for j in 1:N
                    ...
                    i==m && (Tijt[omask, i, j, m] = logaddexp.(view(Tijt,omask,i, j, m), lps.(βT[j],a[m,j],lls[omask,j])))
                end
                for γ in 1:D
...
function ll_func(hmm::AbstractHMM{Univariate}, observation::Vector{Int64})
...
new_b, new_t, new_e = test_bw(hmm, observations, obs_lengths)

old_b, old_t, old_t = old_linear(hmm, Array(transpose(observations)),obs_lengths)
